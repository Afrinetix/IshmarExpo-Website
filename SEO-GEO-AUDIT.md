...
7. **Favicon missing** — `index.html` references `assets/images/favicon.ico`, but no such file exists in `assets/images/`, so the browser tab icon currently 404s on every page. A real favicon (and the modern PNG/SVG + `apple-touch-icon` variants) should be generated from the logo and linked on all 7 pages plus Insights.
